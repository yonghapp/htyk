function ShowPre(o){
	...
	this.deInit = true;
    var wrap_w = $("#slider").width();
    $("#slider .banner-text").width(wrap_w);
    this.li.width(wrap_w);
	...
	if(o.numIco){
		...
		for(var i = 0;i<numL;i++){
				numHtml+="<a href='javascript:void(0);'></a>";
		};
	...
	that.timer = null;
	if(o.auto){
		that.box.parents("#slider").bind("mouseover",function(){
			clearInterval(that.timer);
		});
		that.box.parents("#slider").bind("mouseleave",function(){
			that.autoPlay();
		});
		that.autoPlay();
...
ShowPre.prototype = {
	move:function(test){ //滑动方法
		...
		this.box.stop();
		this.box.stop().animate({left:-pos},600,function(){
			if(test&&that.auto){
				that.autoPlay();
			};
		...
		var img = new Image();
			...
			if(that.deInit){
				...
				if(test!="pre"){
					...
					that.bigBox.css({"margin-left":-100+'%'});
					le = 0;
				};				
			};
