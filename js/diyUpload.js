...
(function( $ ) {
	...
	*/
	//取消事件;	
	function removediv ( $div ,file_id ,webUploader) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//添加子容器;
		var div = '<div id="fileBox_'+file_id+'" class="diyUploadHover"> \
		...
		$fileInput.before( div );		
		var x = $("#house-verifys").children(".diyUploadHover").length;
        var y = $("#house-picboxs").children(".diyUploadHover").length;
        
        if(x >= 2){
          $("#house-verify").css("display","none");
        };
        if(y >= 10){
          $("#house-picbox").css("display","none");
        };
        // document.title = x;
		
		//父容器宽度;
		//var $width = $('.fileBoxUl>li').length * 300;
		//var $maxWidth = $fileInput.parent().width();
		//$width = $maxWidth > $width ? $width : $maxWidth;
		//$parentFileBox.width( $width );
		
		var $fileBox = $('.pic-show').find('#fileBox_'+file_id);

		//绑定取消事件;
		var $diyCancel = $fileBox.children('.diyCancel').on('click',function(){
			removediv( $(this).parent('div'), file_id, webUploader );	
		});
